...
function generateController(upperCase, lowerCase, crudService) {
    ...
    fs.writeFileSync(path.resolve(folder, `${entityName}.controller.metadata.ts`), [
        ...
        crudService ? `import { Controller, ValueObjects } from 'springnext'` : `import { Controller } from 'springnext'`,
    ...
    for (const [currentPath, methods] of Object.entries(nestedMethods)) {
        for (const method of methods) {
            ...
            fs.writeFileSync(fileName, [
                ...
                method === 'GET' 
                    ? [
                        `const queryKey = (payload?: Method['payload']) => payload ? ['${requiredEntity}', ${currentPath.split('/').map(x => `'${x}'`).join(', ')}, normalizeObjectKeysOrder(payload)] : ['${requiredEntity}', ${currentPath.split('/').map(x => `'${x}'`).join(', ')}]`,
                        ``,
                        ...
                        '}',
                        '',
                        `export const use${nameForHook}DataSetting = () => {`,
                        `\tconst queryClient = useQueryClient()`,
                        '',
                        `\treturn (payload: Method['payload'], data: Method['response']) => {`,
                        `\t\tqueryClient.setQueryData(queryKey(payload), data)`,
                        ...
                        '}',
                        '',
                        `export const use${nameForHook}Lazy = () => {`,
                        `\tconst setQueryData = use${nameForHook}DataSetting()`,
                        '\t',
                        `\treturn useMutation<Method['response'], Method['error'], Method['payload']>({`,
